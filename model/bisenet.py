...
class BiSeNet(nn.Module):
    def __init__(self, nclass,inchannel=2, backbone='resnet18', aux=False, jpu=False, pretrained_base=True):
        super(BiSeNet, self).__init__()
        self.aux = aux
        self.spatial_path = SpatialPath(inchannel, 128)
        self.context_path = ContextPath(backbone,inchannel, pretrained_base)
        self.ffm = FeatureFusion(256, 256, 4)
        self.head = _BiSeHead(256, 64, nclass)
        if aux:
            self.auxlayer1 = _BiSeHead(128, 256, nclass)
            self.auxlayer2 = _BiSeHead(128, 256, nclass)
    ...
    def forward(self, x):
        ...
        outputs = F.interpolate(x, size, mode='bilinear', align_corners=True)
        # outputs.append(x)
...
class ContextPath(nn.Module):
    def __init__(self, backbone='resnet18',inchannel=2, pretrained_base=True, norm_layer=nn.BatchNorm2d):
