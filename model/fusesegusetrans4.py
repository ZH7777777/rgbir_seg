...
class fusesegusetrans4(nn.Module):
    def __init__(self, n_class):
        super(fusesegusetrans4, self).__init__()
    ...
    def forward(self, x):
        # ------------ repeat external seq ------------
        batch_size = x.size(0)
        batch_external_seq1 = self.external_seq1.repeat(batch_size, 1, 1) # batch_size * 1104*9
        ...
        # -------- concat external sequence -----------
        image2 = torch.cat([image2, batch_external_seq1], dim=2)   # 1*1104*(70+10)
        thermal2 = torch.cat([thermal2, batch_external_seq2], dim=2)
        # ---------------------------------------------
        ##---------------------------------------------------------------------
        #fusesegusetrans11 最好的那次需要加上两个encoding
        image2 = self.linear_encoding3(image2)
        thermal2 = self.linear_encoding4(thermal2)

        image2 = self.position_encoding(image2)  # 1*1104*79
        image2 = self.pe_dropout(image2)
        thermal2 = self.position_encoding2(thermal2)  # 1*1104*79
        thermal2 = self.pe_dropout(thermal2)
        # image2=image2.permute(0,2,1).contiguous()
        # thermal2 = thermal2.permute(0, 2, 1).contiguous()#1*80*1104
        image2 = self.transformer3(image2)
        image2 = self.pre_head_ln2(image2)
        thermal2 = self.transformer4(thermal2)
        thermal2 = self.pre_head_ln2(thermal2)#1*1104*80




        # --------- drop external sequence ----------
        # image2 = image2.permute(0, 2, 1).contiguous()
        # thermal2 = thermal2.permute(0, 2, 1).contiguous()
        image2 = image2[:, :, :70]
        thermal2 = thermal2[:, :, :70]
        # -------------------------------------------
