...
model_dir = '/content/drive/MyDrive/MFNet-pytorch-master/weights/mffenet_reconrgbir2/'
...
def train(epo, encoder,rec_decoder,se_decoder,rgbbackbone,irbackbone, train_loader, optimizer,optimizer_recon,rgb_gradloss,ir_gradloss,c):
    ...
    for it, (images_yir, labels, names) in enumerate(train_loader):
        ...
        feature=encoder(reconinput)
        # recon=rec_decoder(rgb1,rgb2,rgb3,rgb4)
        recon = rec_decoder(feature)
        logits = se_decoder(feature)
        ...
        loss = 0.02*(20*mse_loss + ssim_loss)+segloss
        # loss = segloss
...
def validation(epo, encoder,se_decoder, val_loader):
    ssimloss = SSIM(window_size=11)
...
def main():
    ...
    encoder=densefuseencoder(input_nc=2)
    rec_decoder=recon_decoder(output_nc=1)
    se_decoder=seg_decoder(output_nc=9)
    ...
    rec_decoder = rec_decoder.to(args.device)
    se_decoder = se_decoder.to(args.device)
    #-------加上lightnet----------------------------------------------------------------------------------
    # lightnet = LightNet()
    # # lightnet.train()
    ...
    optimizer = torch.optim.SGD([
                ...
                {'params': rec_decoder.parameters()},
                {'params': se_decoder.parameters()},

            ], lr=args.lr_start, momentum=0.9, weight_decay=0.0005)
    optimizer_recon = torch.optim.SGD([

        {'params': rgbbackbone.parameters()},
        {'params': irbackbone.parameters()}
    ], lr=args.recon_lr_start, momentum=0.9, weight_decay=0.0005)
    ...
    lr_scheduler_recon = get_scheduler(optimizer_recon, args)

    if args.epoch_from > 1:
        print('| loading checkpoint file %s... ' % checkpoint_model_encoder_file, end='')
        encoder.load_state_dict(torch.load(checkpoint_model_encoder_file))
        rec_decoder.load_state_dict(torch.load(checkpoint_model_rec_decoder_file))
        se_decoder.load_state_dict(torch.load(checkpoint_model_se_decoder_file))
    ...
    for epo in tqdm(range(args.epoch_from, args.epoch_max+1)):
        ...
        train(epo, encoder,rec_decoder,se_decoder,rgbbackbone,irbackbone, train_loader, optimizer,optimizer_recon,rgb_gradloss,ir_gradloss,c)
        lr_scheduler.step()
        lr_scheduler_recon.step()
        if epo > 30 and epo % 2 == 0:
            miou = validation(epo, encoder,se_decoder, val_loader)
        ...
        torch.save(encoder.state_dict(), checkpoint_model_encoder_file)
        torch.save(rec_decoder.state_dict(), checkpoint_model_rec_decoder_file)
        torch.save(se_decoder.state_dict(), checkpoint_model_se_decoder_file)
        torch.save(optimizer.state_dict(), checkpoint_optim_file)
        print('done!')

        if is_best:
            best_miou = miou
            shutil.copy(checkpoint_model_encoder_file, best_model_encoder_file)
            shutil.copy(checkpoint_model_rec_decoder_file, best_model_rec_decoder_file)
            shutil.copy(checkpoint_model_se_decoder_file, best_model_se_decoder_file)
            is_best = 0

    os.rename(checkpoint_model_encoder_file, final_model_encoder_file)
    os.rename(checkpoint_model_rec_decoder_file, final_model_rec_decoder_file)
    os.rename(checkpoint_model_se_decoder_file, final_model_se_decoder_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_start', '-ls', type=float, default=0.006)#0.005
    parser.add_argument('--recon_lr_start', '-rls', type=float, default=0.001)
    ...
    checkpoint_model_encoder_file = os.path.join(model_dir, 'encodertmp.pth')
    checkpoint_model_rec_decoder_file = os.path.join(model_dir, 'rec_decodertmp.pth')
    checkpoint_model_se_decoder_file = os.path.join(model_dir, 'se_decodertmp.pth')
