...
from model import DenseFuse_net,FuseSeg_cat,mffenet_vgg,pool_mffenet,ccaffmnet,mffenetusetrans,mffenet,mffenetusetrans_c,FuseSeguseboundary,mffenetusecswintrans,fusesegusetrans_new,fusesegusetrans5,fusesegusetrans2,MFNet,RTFNet,afnet,FuseSeg,FuNNet,afnetusetrans,fusesegusetrans,fusesegusetrans3,fusesegusetrans4
from train_vgg import n_class, data_dir, model_dir
...
def main():
    ...
    with torch.no_grad():
        ...
        for it, (images_yir, label, names) in enumerate(test_loader):
            images_yir = Variable(images_yir)
            # images = Variable(images)
            # labels = Variable(labels)
            if args.gpu >= 0:
                # images = images.cuda(args.gpu)
                # labels = labels.cuda(args.gpu)
                images_yir=images_yir.cuda(args.gpu)
                # y_label=y_label.cuda(args.gpu)
                # ir_label = ir_label.cuda(args.gpu)
            ...
            cc_rgb = pearsonr(reconstruct.view(b,c,-1),images_yir[:,:3].view(b,c,-1))
            ...
            psnrrgb=PSNR(images_yir[:,:3], reconstruct)
            ...
            filepath = '/content/drive/MyDrive/MFNet-pytorch-master/weights/mffenet_reconrgbir/output/' + names[0] + '.png'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name',  '-M',  type=str, default='densefuse')
