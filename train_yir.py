...
model_dir = '/content/drive/MyDrive/MFNet-pytorch-master/weights/bisenet/'
...
def train(epo, model,train_loader, optimizer):
    ...
    # for it, (images_yir,y_label,ir_label, names) in enumerate(train_loader):
        ...
        y=images[:,:1]
        ir=images[:,3:]
        input=torch.cat((y,ir),dim=1)
        # y=torch.cat((images_yir[:, :1], images_yir[:, :1], images_yir[:, :1]), dim=1)
        # rgb=images[:, :3]
        # ir = torch.cat((images_yir[:, 1:], images_yir[:, 1:], images_yir[:, 1:]), dim=1)
...
def validation(epo, model, val_loader):
    ...
    with torch.no_grad():
        ...
        # for it, (images_yir,y_label,ir_label, names) in enumerate(val_loader):
            ...
            # ycbcr=images[:,:3]
            logits = model(input)
            # mse1 = mse(reconstruct,images_yir[:,:1])
            # mse2 = mse(reconstruct,images_yir[:,1:])
            # loss = F.cross_entropy(logits, labels)
            pred = F.softmax(logits, dim=1)
            loss = L.lovasz_softmax(pred, labels)
            acc = calculate_accuracy(logits, labels)
            # loss=mse1+mse2
            # SSIM_rgb = ssimloss(images_yir[:,:1], reconstruct)
            # SSIM_ir = ssimloss(images_yir[:,1:], reconstruct)
            # ssim = (SSIM_rgb + SSIM_ir) / 2.0
            # b, c, h, w = reconstruct.size()
            # cc_rgb = pearsonr(reconstruct.view(b, c, -1), y.view(b, c, -1))
            # print(images_yir[:,:1].shape)
            # cc_ir = pearsonr(reconstruct.view(b, c, -1), y.view(b, c, -1))
            # cc = (cc_rgb + cc_ir) / 2.0
            # print(cc.shape)
            # cc=torch.mean(cc)
            ...
            # psnrrgb = PSNR(images_yir[:,:1], reconstruct)
            # psnrir = PSNR(images_yir[:,1:], reconstruct)
...
def main():
    ...
    model=BiSeNet(nclass=9,inchannel=2,backbone='resnet18')
    # model = DenseFuse_net(input_nc=2,output_nc=1)
    # if args.gpu >= 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size_val', '-Bv', type=int, default=8)
    ...
    parser.add_argument('--lr_start', '-ls', type=float, default=0.01)#0.005
