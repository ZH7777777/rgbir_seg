...
def train(epo, model,rgbbackbone,irbackbone, train_loader, optimizer,rgb_gradloss,ir_gradloss,c):
    ...
    for it, (images_yir, label, names) in enumerate(train_loader):
        ...
        rgb=images_yir[:,:3]
        rgb_backbone_feature=rgbbackbone(rgb)
        ...
        ir_grad=ir_gradloss(ir_backbone_feature)/1500
        ...
        SSIM_rgb = 1 - ssimloss(reconstruct,rgb)
        SSIM_ir = 1 - ssimloss(reconstruct,ir)
        mse1=mse(rgb,reconstruct)
        mse2 = mse(ir,reconstruct)
...
def validation(epo, model, val_loader):
    ...
    with torch.no_grad():
        ...
        for it, (images_yir, label, names) in enumerate(val_loader):
...
def main():
    ...
    for epo in tqdm(range(args.epoch_from, args.epoch_max+1)):
        ...
        train(epo, model,rgbbackbone,irbackbone, train_loader, optimizer,rgb_gradloss,ir_gradloss,c)
        lr_scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size_val', '-Bv', type=int, default=1)
