...
class RandomScale(object):
    ...
    def __call__(self, img,label):
        im = img
        lb = label
        W, H = np.size(im,1),np.size(im,0)
        scale = random.choice(self.scales)
        w, h = int(W * scale), int(H * scale)
        im = np.asarray(Image.fromarray(np.asarray(im,dtype=np.uint8)).resize((w, h), Image.BILINEAR),dtype=np.float32)
        lb = np.asarray(Image.fromarray(np.asarray(lb)).resize((w, h), Image.NEAREST),dtype=np.int64)
        return im,lb
...
class RandomSized(object):
    ...
    def __call__(self, img, mask):
        ...
        # b=self.scale(img, mask)
        # a=self.crop(*self.scale(img, mask))
